...
+
. A new window will pop out where you can edit the amount of CPU and Memory. For our XL template set the value of CPUs to 4, and Memory to 8 GiB, and click the *Save* button.
+
...
+
NOTE: Notice that there is initially no option to quick create this VM, and we must customize the VM to fit our needs.
+
...
+
. Note that the *Use this disk as a boot source* option has been automatically enabled. Click on the "Save" button to accept this configuration.
+
