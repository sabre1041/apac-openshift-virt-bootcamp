...
+
. Using the *Actions* dropdown menu or the shortcut button in the top right corner, *Stop* the VM. This process can take a long time since it attempts a graceful shutdown and the machine is in an unstable state. If you click on the *Actions* dropdown menu again and then select *Control*, you will have the option to select *Force stop*. Please make use of this option in order to continue with the lab. 
+
...
+
. Return to *Overview* tab, and start the VM using the *Start* button or from within the *Actions* dropdown menu.
+
