...
+
. Repeat the above steps to add the *vlan0* interface to the *fedora02* VM as well. Be sure to restart the VM after adding the interface so that the changes take effect.
...
[source,sh]
----
$ sudo nmcli con mod 'Wired connection 1' ipv4.method manual ipv4.addresses 192.168.69.238/18
...
+
. Repeat the above steps on *fedora02*, but give it a different IP address within the same subnet.
+
...
[source,sh]
----
$ ping -c3 192.168.69.238
PING 192.168.69.238 (192.168.69.238) 56(84) bytes of data.
64 bytes from 192.168.69.238: icmp_seq=1 ttl=64 time=3.33 ms
64 bytes from 192.168.69.238: icmp_seq=2 ttl=64 time=0.957 ms
64 bytes from 192.168.69.238: icmp_seq=3 ttl=64 time=0.930 ms

--- 192.168.69.238 ping statistics ---
3 packets transmitted, 3 received, 0% packet loss, time 2003ms
rtt min/avg/max/mdev = 0.930/1.737/3.325/1.122 ms

$ ping -c3 192.168.69.239
PING 192.168.69.239 (192.168.69.239) 56(84) bytes of data.
64 bytes from 192.168.69.239: icmp_seq=1 ttl=64 time=3.12 ms
64 bytes from 192.168.69.239: icmp_seq=2 ttl=64 time=1.03 ms
64 bytes from 192.168.69.239: icmp_seq=3 ttl=64 time=0.870 ms

--- 192.168.69.239 ping statistics ---
3 packets transmitted, 3 received, 0% packet loss, time 2003ms
...
----
+
. Attempt to ping *fedora01*  from the bastion host. This attempt should still fail as we have not explicitly allowed it.
+
