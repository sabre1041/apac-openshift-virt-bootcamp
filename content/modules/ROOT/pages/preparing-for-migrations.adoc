...
Locate the Job Template named *Configure Networks - vmware-virt-bootcamp - openshift-virt.bootcamp.redhat.com* and select the rocket icon.

image::Ansible-Migration-Prep/aap-job-templates-nmstate.png[link=self, window=blank, width=100%]

This job template is configured to allow additional variables to be specified at launch time. Since the Bootcamp OpenShift environment includes a customized network set up, we will define how the automation creates the associated Nmstate resources.
...
A _Workflow Job Template_ called *Configure Target - vmware-virt-bootcamp - openshift-virt.bootcamp.redhat.com* is available to initialize individual Namespaces, such as the `vmexamples-automation` Namespace, with the remaining set of resources the is needed to support Migration activities including the NetworkMaps, StorageMaps, and credentials for the VDDK image.
...
Click the Rocket icon next to the *Configure Target - vmware-virt-bootcamp - openshift-virt.bootcamp.redhat.com* Workflow Job Template which, like the Nmstate Job Template, allow for additional Ansible variables to be specified.
...
Confirm the *vmware-virt-bootcamp-vddk* _Secret_ is present in the namespace.

image::Ansible-Migration-Prep/openshift-console-namespace-secrets.png[link=self, window=blank, width=100%]

Review the _NetworkMaps_ and _StorageMaps_ that were also created in the `vmexamples-automation` Namespace. If you look at the *vmware-virt-bootcamp-host* _NetworkMap_ within the *Network maps* page under the _Migration for Virtualization_ section of the left hand navigation bar, select the *YAML* tab to view the details of the resource.
...
The *vmware-virt-bootcamp-host* _StorageMap_ on the *Storage maps* page was configured with the connection to the `ocs-external-storagecluster-ceph-rbd` as it is the default _StorageClass_ in the cluster.
