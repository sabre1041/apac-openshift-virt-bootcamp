...
+
.. On the left hand side, right click *Local Cluster* and then click *Create Project*.
+
image::module-00/02_VM_Dialog_Create_Project.png[link=self, window=blank, width=100%]
+
...
+
. After creating the project, it will not be displayed in the dialog as no Virtual Machines have been created in it. To view all projects regardless of whether they have VMs in them, deselect the *Show only projects with VirtualMachines* checkbox to reveal the project created previously.
+
image::module-00/04_Reveal_All_Projects.png[link=self, window=blank, width=100%]
...
+
image::module-00/05_Create_VM_Button.png[link=self, window=blank, width=100%]

. The wizard will appear showing the available pre-defined VM templates.
+
Reviewing the list of available templates you'll notice that some have a blue badge which indicates "Source available". These are templates which are using the automatically downloaded and stored template disks. If you were deploying in your own environment, you have the option of preventing these from being created and/or removing those source disks, followed by creating and uploading custom disks for your organization.
+
image::module-00/06_Create_VM_Templates.png[link=self, window=blank, width=100%]

. Ensure that the `vmexamples` project is selected in the dropdown at the top of the dialog, so that the VM will be created in the project created in the previous section.
+
. Select the *Fedora VM* tile, and a dialog opens.
+
image::module-00/07_Create_VM_Quick.png[link=self, window=blank, width=100%]

. Change the name to `fedora01` and press *Quick create VirtualMachine*:
+
image::module-00/08_Create_VM_Quick_Name.png[link=self, window=blank, width=100%]
...
+
image::module-00/09_Fedora_Running.png[link=self, window=blank, width=100%]
...
+
image::module-00/10_Fedora_Events.png[link=self, window=blank, width=100%]
+
...
+
image::module-00/11_Fedora_Details.png[link=self, window=blank, width=100%]
...
. Click the *Console* tab to access the VM's console. Note that login credentials are provided on this page when they have been injected into the guest via `cloud-init`. Click the eye icon to reveal the username and password that can be used to log in to the VM.
+
...
[IMPORTANT]
====
You can click on `Paste to Console` to paste contents of your clipboard into the Virtual Machine.
====
+
[IMPORTANT]
...
+
image::Create_VM_PVC/19_Fedora_Agent_Details_Network_Storage.png[link=self, window=blank, width=100%]
+
* Hostname
...
[NOTE]
====
You may have to make your browser window wider to see the node's full name. Alternatively, you can collapse the Virtual Machine hierarchy on the left side of the screen to enable more dialogs to be visible.
====
...
+
. A dialog will appear presenting the option of automatically selecting the node the VM will be migrated to or allowing you to select the destination node. For this lab, choose *Automatically selected Node* and click *Migrate Virtual Machine*.
+
image::module-00/21_VM_Dialog_Migrate_Choose_Node.png[link=self, window=blank, width=100%]
+
...
. Clicking on *Actions* and then *Control*, the option *Start* appears, and the options *Restart* and *Pause* are greyed out.
+
