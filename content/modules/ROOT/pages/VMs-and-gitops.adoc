...
The course git repo is located at `{bootcamp_git_repo}`.
...
Cloning into 'apac-openshift-virt-bootcamp'...
...
Set an environment variable named `BOOTCAMP_REPO` with the path to the cloned repository. Since you should be in the root of the cloned repository, you can set this variable with the following command:
...
[source,console,subs="attributes"]
----
$ oc apply -f $BOOTCAMP_REPO/content/gitops/bootstrap/openshift-gitops-namespace.yaml
namespace/openshift-gitops created

$ oc apply -f $BOOTCAMP_REPO/content/gitops/bootstrap/subscription.yaml
subscription.operators.coreos.com/openshift-gitops-operator created

$ oc apply -f $BOOTCAMP_REPO/content/gitops/bootstrap/cluster-rolebinding.yaml
clusterrolebinding.rbac.authorization.k8s.io/cluster-admin-gitops-sc created

$ oc apply -f $BOOTCAMP_REPO/content/gitops/bootstrap/console-plugin.yaml
...
On the left hand navigation, select *Ecosystem* -> *Installed Operators* in your OpenShift console to verify that the *Red Hat OpenShift GitOps* operator was successfully installed. Ensure that *All Projects* is selected in the dropdown menu at the top of the page so that the operator is visible.
...
When prompted to authorize access, click on *Allow selected permissions*.  The Argo CD console will be shown.

image::VMs-and-gitops/argocd-console.png[link=self, window=blank, width=100%]

## Deploy the Argo CD Instance

Let's customize the Argo CD instance, However, in a real world scenario , you might have to design for multitenancy, in which case you might end up needing more than one instance. Run the following commands to set a few environment variables based on the bootcamp environment.

[source,console,subs="attributes"]
----
$ export gitops_repo={bootcamp_git_repo}
...
In the Argo CD console, verify that the two child applications have been correctly deployed (It may take a minute or so for the status of both applications to become healthy and synced).
...
Within your OpenShift console, log in to the console of the `fedora-apache` VM in the `gitops-test` namespace by navigating to *Virtualization* -> *Virtual Machines* and selecting the `fedora-apache` VM.

Navigate to the *Console* tab and log in using the guest credentials provided.

Execute the following commands to install and enable httpd on the VM:
...
In this example, one can also use the K8s services' fully qualified domain names instead of omitting name resolution by using explicitly the IP addresses.
...
So, you can also run the following command from the bastion host that only relies on names, not on actual IP addresses:
